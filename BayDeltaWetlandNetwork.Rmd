...
#This step is only needed for clipping original raw data to obtain the bio and ces shapefiles that are for bay-delta counties (original data is for all of CA)
...
#code below shows how original CES data was clipped to create 'ces' file
#load in all CA calenviroscreen tracts
#ces_ca <- st_read("CES4 Final Shapefile.shp")
...
#load 'CES' data here
ces <- st_read("ces.shp")
ces <- ces %>% st_transform(crs="+proj=longlat +datum=WGS84 +ellps=WGS84")

```

```{r load biodiversity data}
#Code below shows how original data was clipped to create 'bio' data
#load biodiversity data from CDFW
#fullbio <- st_read("Species_Biodiversity_ACE_[ds2769].shp")
...
#Load in clipped biodiversity data here
bio <- st_read("bio.shp")
bio <- bio %>% st_transform(crs= "+proj=longlat +datum=WGS84 +ellps=WGS84")
...
CalEnviro.aw <- aw_interpolate(projects.utm, tid = shpid, source = ces.utm, sid = Tract,
                           ...
                           intensive = c("CIscore"))
#drop geometry again
projCES <- CalEnviro.aw %>% st_drop_geometry() %>% dplyr::select("shpid", "pname", "CIscore")
...
projcoordsFLOOD <- read.csv("projcoordsfloodrisk.csv") #flood depth calculated for 14 projects with coordinates
projboundFLOOD <- read.csv("projboundsfloodrisk.csv") #flood depth calculated for 156 projects with shapefile boundaries
...
#attributes2 <- attributes2 %>% rename(EAid = EAid.x) #rename if merge renamed EAid column
...
set.seed(9199919)
#Control Model
TERGM_control <- btergm(estnets ~ edges + gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5, fixed=TRUE) + b2degree(1), offset=FALSE)
...
plot(tergmgof_control[[5]])

#TC model
TERGM_TC <- btergm(estnets ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1)+ b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("fundingpools") + b2nodematch("gov") + b2nodematch("orgscale"), offset=FALSE)
summary(TERGM_TC)

#Goodness of Fit
tergmgof_TC <- btergm::gof(TERGM_TC)
plot(tergmgof_TC[[3]], xlim=15) #plot of degree distribution
plot(tergmgof_TC[[4]], xlim=10) #plot of geodesic distances
plot(tergmgof_TC[[5]])

#Project benefits
TERGM_bens <- btergm(estnets ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b1cov("totalES") + b1cov("tidal") + b1cov("delta"), offset=FALSE)
summary(TERGM_bens)

#Full model
TERGM_full <- btergm(estnets ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2factor("orgscale") + b2cov("gov") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("fundingpools") + b2nodematch("orgscale") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta"), offset=FALSE)
summary(TERGM_full)
...
set.seed(1111)
#Control TERGM
TERGM_spatialcontrol <- btergm(estspatialnets ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1), offset=FALSE)
summary(TERGM_spatialcontrol)
btergm.se(TERGM_spatialcontrol)

#general goodness of fit
tergmgof_spatialcontrol <- btergm::gof(TERGM_spatialcontrol)
plot(tergmgof_spatialcontrol[[3]], xlim=15)
plot(tergmgof_spatialcontrol[[4]], xlim=15)

#TC model
TERGM_spatialTC <- btergm(estspatialnets ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1)+ b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("fundingpools") + b2nodematch("gov") + b2nodematch("orgscale"), offset=FALSE)
summary(TERGM_spatialTC)

#Project benefits
TERGM_spatialbens <- btergm(estspatialnets ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_spatialbens)

tergmgof_spatialben <- btergm::gof(TERGM_spatialbens)
plot(tergmgof_spatialben[[3]], xlim=15)
plot(tergmgof_spatialben[[4]], xlim=10)
plot(tergmgof_spatialben[[5]])

#Full model
TERGM_spatialfull <- btergm(estspatialnets ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("fundingpools") + b2nodematch("orgscale") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_spatialfull)

tergmgof_spatialfull <- btergm::gof(TERGM_spatialfull)
plot(tergmgof_spatialfull[[3]], xlim=15)
plot(tergmgof_spatialfull[[4]], xlim=10)
plot(tergmgof_spatialfull[[5]])
```

```{r extra TERGMs}
#Model with spatial variables without org scale
TERGM_spatialfull_noscale <- btergm(estspatialnets ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("fundingpools") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_spatialfull_noscale)

#p-values for full model
TERGM_spatialfull_noscale_se <- btergm.se(TERGM_spatialfull_noscale)
#now contains the p-values in the fourth column
TERGM_spatialfull_noscale_p_values <- TERGM_spatialfull_noscale_se[, 4]

TERGM_spatialfull_noscale_p_values

#Model without spatial variables without org scale
TERGM_full_noscale <- btergm(estnets ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("fundingpools") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta"), offset=FALSE)
summary(TERGM_full_noscale)

#p-values for full model
TERGM_full_noscale_se <- btergm.se(TERGM_full_noscale)
#now contains the p-values in the fourth column
TERGM_full_noscale_p_values <- TERGM_full_noscale_se[, 4]

TERGM_full_noscale_p_values
```

####Decay Value Checks-------
```{r decay value check}
TERGM_control1<- btergm(estnets ~ edges + gwb1degree(1, fixed=TRUE)+gwb2degree(1, fixed=TRUE) + b2degree(1), offset=FALSE)
summary(TERGM_control1)

TERGM_control0.75 <- btergm(estnets ~ edges + gwb1degree(0.75, fixed=TRUE)+gwb2degree(0.75, fixed=TRUE) + b2degree(1), offset=FALSE)
summary(TERGM_control0.75)

#0.5 degree used above
summary(TERGM_control)

TERGM_control0.25 <- btergm(estnets ~ edges + gwb1degree(0.25, fixed=TRUE)+gwb2degree(0.25, fixed=TRUE) + b2degree(1), offset=FALSE)
summary(TERGM_control0.25)

TERGM_control1.25 <-  btergm(estnets ~ edges + gwb1degree(1.25, fixed=TRUE)+gwb2degree(1.25, fixed=TRUE) + b2degree(1), offset=FALSE)
summary(TERGM_control1.25)

#general goodness of fit
tergmgof_control0.25 <- btergm::gof(TERGM_control0.25)
tergmgof_control<- btergm::gof(TERGM_control)
tergmgof_control0.75 <- btergm::gof(TERGM_control0.75)
tergmgof_control1 <- btergm::gof(TERGM_control1)
tergmgof_control1.25 <- btergm::gof(TERGM_control1.25)

#compare degree distribution plots for decay values
plot(tergmgof_control0.25[[3]], xlim=15)
plot(tergmgof_control[[3]], xlim=15)
plot(tergmgof_control0.75[[3]], xlim=15)
plot(tergmgof_control1[[3]], xlim=15)
plot(tergmgof_control1.25[[3]], xlim=15)

#compare geodesic distances
plot(tergmgof_control0.25[[4]], xlim=10)
plot(tergmgof_control[[4]], xlim=10)
plot(tergmgof_control0.75[[4]], xlim=10)
plot(tergmgof_control1[[4]], xlim=10)
plot(tergmgof_control1.25[[4]], xlim=10)
```
...
PlainERGMcontrol <- ergm(net ~ edges + gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5, fixed=TRUE) + b2degree(1))
summary(PlainERGMcontrol)

PlainERGM <- ergm(net ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2factor("orgscale") + b2cov("gov") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("fundingpools") + b1cov("totalES") + b1cov("tidal") + b1cov("delta"))
summary(PlainERGM)

PlainERGM_spatial <- ergm(spatialnet ~edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("fundingpools") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"))
summary(PlainERGM_spatial)

```

#### Bonferroni Correction--------
```{r bonferroni}
#recheck if significance levels are above 0.05/14 (0.0036) in TERGM_spatialfull, TERGM_spatialbens , and TERGM_spatialTC
#put in corrected (divide by 14) and regular p values

#obtain p values
#for full model
TERGM_spatialfull_se <- btergm.se(TERGM_spatialfull)

#now contains the p-values in the fourth column
TERGM_spatialfull_p_values <- TERGM_spatialfull_se[, 4]

TERGM_spatialfull_p_values

TERGM_spatialfull_stde <- TERGM_spatialfull_se[, 2]
TERGM_spatialfull_stde

#identify which remain significant after the bonferroni correction (i.e. p value < 0.0036)

TERGM_spatialfull_sigbonf <- ifelse(TERGM_spatialfull_p_values <0.0036, TERGM_spatialfull_p_values, "not sig")

TERGM_spatialfull_sigbonf  

#redo for TC model
TERGM_spatialTC_se <- btergm.se(TERGM_spatialTC)

#now contains the p-values in the fourth column
TERGM_spatialTC_p_values <- TERGM_spatialTC_se[, 4]

TERGM_spatialTC_p_values

TERGM_spatialTC2_stde <- TERGM_spatialTC2_se[, 2]

TERGM_spatialTC2_stde

#identify which remain significant after the bonferroni correction (i.e. p value < 0.0036)

TERGM_spatialTC_sigbonf <- ifelse(TERGM_spatialTC_p_values <0.0036, TERGM_spatialTC_p_values, "not sig")

TERGM_spatialTC_sigbonf


#redo for benefits model
TERGM_spatialbens_se <- btergm.se(TERGM_spatialbens)

#now contains the p-values in the fourth column
TERGM_spatialbens_p_values <- TERGM_spatialbens_se[, 4]

TERGM_spatialbens_p_values

TERGM_spatialbens_stde <- TERGM_spatialbens_se[, 2]
TERGM_spatialbens_stde

#identify which remain significant after the bonferroni correction (i.e. p value < 0.0036)

TERGM_spatialbens_sigbonf <- ifelse(TERGM_spatialbens_p_values <0.0036, TERGM_spatialbens_p_values, "not sig")

TERGM_spatialbens_sigbonf

```

#### Correlations----------
```{r org scale and type}
#Code from https://rpubs.com/hoanganhngo610/558925
table(attributes2$orgtype2, attributes2$orgscale2)
chisq.test(attributes2$orgtype2, attributes2$orgscale2)
library('DescTools')
ContCoef(attributes2$orgtype2, attributes2$orgscale2, correct = FALSE)
ContCoef(attributes2$orgtype2, attributes2$orgscale2, correct = TRUE)

library('rcompanion')
cramerV(attributes2$orgtype2, attributes2$orgscale2, bias.correct = FALSE)

library('lsr')
cramersV(attributes2$orgtype2, attributes2$orgscale2)

library('vcd')
assocstats(xtabs(~attributes2$orgtype2 + attributes2$orgscale2))

rcompanion::cramerV(attributes2$orgtype2, attributes2$orgscale2, bias.correct = TRUE)

DescTools::UncertCoef(table(attributes2$orgtype2, attributes2$orgscale2), direction = "column")

UncertCoef(table(attributes2$orgtype2, attributes2$orgscale2), direction = "row")

UncertCoef(table(attributes2$orgtype2, attributes2$orgscale2), direction = "symmetric")
```

```{r project attribute correlations}
#run correlations between predictor variables

#project correlations- total ES, CES, biodiversity, flood risk, delta, tidal, thous acres

#correlation matrices
projattscorr <- attributes2 %>% select(tidal, deltaregion, SUM_benefits, finFLOOD3, finBIO, finCES, thousacres)
projattscorr <- projattscorr[1:225, ]
projattcorrs <- correlate(projattscorr)
projattcorrsdf <- as.data.frame(projattcorrs[[1]])

#correlation plot
library(corrplot)
row.names(projattcorrsdf)= c("tidal", "deltaregion", "SUM_benefits", "finFLOOD3", "finBIO", "finCES", "thousacres")
projattcorrsdf <- as.matrix(projattcorrsdf)

rownames(projattcorrsdf) <- c("Tidal", "Delta", "TotalES", "FloodRisk", "Biodiversity", "CalEnviroScreen", "Acreage")

colnames(projattcorrsdf) <- c("Tidal", "Delta", "TotalES", "FloodRisk", "Biodiversity", "CalEnviroScreen", "Acreage")

corrplot(projattcorrsdf, method="circle", diag=FALSE)

library(Hmisc)
indivprojattcorrs <- rcorr(as.matrix(projattscorr))

projattcoeffs <- rcorr(as.matrix(projattscorr), type="spearman")[[1]][1:7, 1:7]
projattcoeffs

projattpvalues <- rcorr(as.matrix(projattscorr), type="spearman")[[3]][1:7, 1:7]
projattpvalues <- as.data.frame(projattpvalues)
projattpvalues
```

```{r org attribute correlations}
#org correlations- org scale, gov org type, implementer, funding pools, boundary spanner

#correlation matrices
orgattscorr <- attributes2 %>% select(govorg, implementer, fundpools, orgboundaryspanner, localorg, degree)
orgattscorr <- orgattscorr[226:394, ]
orgattcorrs <- correlate(orgattscorr)
orgattcorrsdf <- as.data.frame(orgattcorrs[[1]])

#correlation plot
library(corrplot)
row.names(orgattcorrsdf)=c("govorg", "implementer", "fundpools", "orgboundaryspanner", "localorg", "degree")
orgattcorrsdf <- as.matrix(orgattcorrsdf)

rownames(orgattcorrsdf) <- c("Gov", "Implementer", "Funding Pools", "Boundary Spanner", "Local", "Degree")

colnames(orgattcorrsdf) <- c("Gov", "Implementer", "Funding Pools", "Boundary Spanner", "Local", "Degree")

corrplot(orgattcorrsdf, method="circle", diag=FALSE)

indivorgattcorrs <- rcorr(as.matrix(orgattscorr))

orgattcoeffs <- rcorr(as.matrix(orgattscorr), type="spearman")[[1]][1:6, 1:6]
orgattcoeffs

orgattpvalues <- rcorr(as.matrix(orgattscorr), type="spearman")[[3]][1:6, 1:6]
orgattpvalues <- as.data.frame(orgattpvalues)
orgattpvalues
```

####Additional Descriptives---------
```{r org type degree}
#step0net, step1net, step2net, step3net, step4net, step5net, step6net degree by orgtype
#will need to calculate two mode degree for actors for each network and then assign attribute and group by org type

#actor org types

orgtypes <- attributes2 %>% select(nodes, orgtype, orgtype2, orgscale)
orgtypes <- orgtypes[226:394, ]
orgtypes <-  orgtypes %>% rename(org=nodes)


#step0net
step0edgelist <- step0edge
step0edgelist$weight <- 1
step0degree <- step0edgelist %>% group_by(org) %>% summarise(degree=sum(weight))
#merge degrees with org types
step0degree <- left_join(step0degree, orgtypes, by="org")
step0degree %>% group_by(orgtype2) %>% summarise(mean(degree))


#step1net
step1edgelist <- step1edge
step1edgelist$weight <- 1
step1degree <- step1edgelist %>% group_by(org) %>% summarise(degree=sum(weight))
step1degree <- left_join(step1degree, orgtypes, by="org")
step1degree %>% group_by(orgtype2) %>% summarise(mean(degree))

#step2net
step2edgelist <- step2edge
step2edgelist$weight <- 1
step2degree <- step2edgelist %>% group_by(org) %>% summarise(degree=sum(weight))
step2degree <- left_join(step2degree, orgtypes, by="org")
step2degree %>% group_by(orgtype2) %>% summarise(mean(degree))

#step3net
step3edgelist <- step3edge
step3edgelist$weight <- 1
step3degree <- step3edgelist %>% group_by(org) %>% summarise(degree=sum(weight))
step3degree <- left_join(step3degree, orgtypes, by="org")
step3degree %>% group_by(orgtype2) %>% summarise(mean(degree))

#step4net
step4edgelist <- step4edge
step4edgelist$weight <- 1
step4degree <- step4edgelist %>% group_by(org) %>% summarise(degree=sum(weight))
step4degree <- left_join(step4degree, orgtypes, by="org")
step4degree %>% group_by(orgtype2) %>% summarise(mean(degree))

#step5net
step5edgelist <- step5edge
step5edgelist$weight <- 1
step5degree <- step5edgelist %>% group_by(org) %>% summarise(degree=sum(weight))
step5degree <- left_join(step5degree, orgtypes, by="org")
step5degree %>% group_by(orgtype2) %>% summarise(mean(degree))

#step6net
step6edgelist <- step6edge
step6edgelist$weight <- 1
step6degree <- step6edgelist %>% group_by(org) %>% summarise(degree=sum(weight))
step6degree <- left_join(step6degree, orgtypes, by="org")
step6degree %>% group_by(orgtype2) %>% summarise(mean(degree))

```

```{r funding pools}
mean(attributes2$fundpools, na.rm=TRUE)
sd(attributes2$fundpools, na.rm=TRUE)
median(attributes2$fundpools, na.rm=TRUE)
table(attributes2$fundpools)


#edgelist with funder names
fundertotals <- EcoAtlas %>% select(fundername, fundingamount, fundingid)
#get total funding for each org

funders_sums <- fundertotals %>% group_by(fundername) %>% summarise(totalbudget=sum(fundingamount))

funders_sums$USFWS <- ifelse(grepl("USFWS", funders_sums$fundername), 'USFWS', "NA")

funders_sums %>% group_by(USFWS) %>% summarise(USFWSfund=sum(totalbudget))

funders_sums$WCB <- ifelse(grepl("WCB", funders_sums$fundername), 'WCB', "NA")

funders_sums %>% group_by(WCB) %>% summarise(WCBfund=sum(totalbudget))

funders_sums$SCC <- ifelse(grepl("SCC", funders_sums$fundername), 'SCC', "NA")
funders_sums %>% group_by(SCC) %>% summarise(SCCfund=sum(totalbudget))
```

```{r additional centrality}
#Check centrality of all state/regional, national scale, and government orgs

stateregorgs <- attributes2 %>% filter(orgscale2=="regional/state")

hist(stateregorgs$degree, breaks=50, xlab="Two Mode Degree", main="State/Regional Org Degree (N=43)")

nationalorgs <- attributes2 %>% filter(orgscale2=="national/other")


hist(nationalorgs$degree, breaks=50, xlab="Two Mode Degree", main="National/Other Org Degree (N=62)")

govorgs <- attributes2 %>% filter(govorg=='1')


hist(govorgs$degree, breaks=50, xlab="Two Mode Degree", main="Gov Org Degree (N=82)")

#Delta Versus Bay Org Centrality
bayatts <- attributes2 %>% filter(attributes2$nodes %in% baynodes)
bayatts <- bayatts %>% filter(!is.na(orgtype)) #161 orgs

deltaatts <- attributes2 %>% filter(nodes %in% deltanodes)
deltaatts <- deltaatts %>% filter(!is.na(orgtype)) #19 orgs

summary(bayatts$degree)
summary(deltaatts$degree)

table(deltaatts$orgscale2)
table(bayatts$orgscale2)
table(deltaatts$govorg)
table(bayatts$govorg)

```
```{r create condensed network figure}
#create network of 3 time steps for policy brief 

edges3$timestepV2 <- ifelse(edges3$startyear<1996 & edges3$startyear>1981, 1, ifelse(edges3$startyear<2009 & edges3$startyear>1995, 2, ifelse(edges3$startyear >2008 & edges3$startyear<2023, 3, NA)))


#split up the edgelists by timesteps
#combine edgelists so each sequential time step is all previous ties
#step0- 1980 to 1987
step1edgeV2 <- edges3 %>% filter(timestepV2==1)

#step1- 1988 to 1993
step2edgeV2 <- edges3 %>% filter(timestepV2==2)

#step2- 1994 to 1999
step3edgeV2 <- edges3 %>% filter(timestepV2==3)

#subset the network into these time steps

#step 1 
step1nodesV2 <- c(step1edgeV2$EAid, step1edgeV2$org)
step1nodesV2 <- as.vector(step1nodesV2)
step1nodesV2 <- as.data.frame(step1nodesV2)
step1nodesV2 <- distinct(step1nodesV2)
step1nodesV2 <- as.vector(step1nodesV2)
step1nodesV2 <- unlist(step1nodesV2)
step1netV2 <- get.inducedSubgraph(net,v=which(net%v%'vertex.names' %in% step1nodesV2))

#step 2 
step2nodesV2 <- c(step2edgeV2$EAid, step2edgeV2$org)
step2nodesV2 <- as.vector(step2nodesV2)
step2nodesV2 <- as.data.frame(step2nodesV2)
step2nodesV2 <- distinct(step2nodesV2)
step2nodesV2 <- as.vector(step2nodesV2)
step2nodesV2 <- unlist(step2nodesV2)
step2netV2 <- get.inducedSubgraph(net,v=which(net%v%'vertex.names' %in% step2nodesV2))


#step 3 
step3nodesV2 <- c(step3edgeV2$EAid, step3edgeV2$org)
step3nodesV2 <- as.vector(step3nodesV2)
step3nodesV2 <- as.data.frame(step3nodesV2)
step3nodesV2 <- distinct(step3nodesV2)
step3nodesV2 <- as.vector(step3nodesV2)
step3nodesV2 <- unlist(step3nodesV2)
step3netV2 <- get.inducedSubgraph(net,v=which(net%v%'vertex.names' %in% step3nodesV2))


#plot each network
sna::gplot(step1netV2, 
           gmode="twomode", 
           usearrows = FALSE,
           edge.col="gray", vertex.cex=2, displayisolates = T, jitter=T)

sna::gplot(step2netV2, 
           gmode="twomode", 
           usearrows = FALSE,
           edge.col="gray", vertex.cex=2, displayisolates = T, jitter=T)

sna::gplot(step3netV2, 
           gmode="twomode", 
           usearrows = FALSE,
           edge.col="gray", vertex.cex=2, displayisolates = T, jitter=T)
...
```{r Data Filtering Check}
#Check for unique project IDs in each data filtering step

RawEcoAtlas2 <- RawEcoAtlas2 %>% 
  unite(EAid, projectid, siteid, activityid, sep = ".", remove = FALSE)
length(unique(RawEcoAtlas2$EAid))


RawEcoAtlas3 <- RawEcoAtlas3 %>% 
  unite(EAid, projectid, siteid, activityid, sep = ".", remove = FALSE)
length(unique(RawEcoAtlas3$EAid))


RawEcoAtlas4 <- RawEcoAtlas4 %>% 
  unite(EAid, projectid, siteid, activityid, sep = ".", remove = FALSE)
length(unique(RawEcoAtlas4$EAid))


RawEcoAtlas5 <- RawEcoAtlas5 %>% 
  unite(EAid, projectid, siteid, activityid, sep = ".", remove = FALSE)
length(unique(RawEcoAtlas5$EAid))

RawEcoAtlas6 <- RawEcoAtlas6 %>% 
  unite(EAid, projectid, siteid, activityid, sep = ".", remove = FALSE)
length(unique(RawEcoAtlas6$EAid))

RawEcoAtlas7 <- RawEcoAtlas7 %>% 
  unite(EAid, projectid, siteid, activityid, sep = ".", remove = FALSE)
length(unique(RawEcoAtlas7$EAid))

EcoAtlas <- EcoAtlas %>% 
  unite(EAid, projectid, siteid, activityid, sep = ".", remove = FALSE)
length(unique(EcoAtlas$EAid))
```

```{r additional GOF}
#for each of the 4 TERGMs calculate AUC of PR curves and ROC and compare to baseline network density (from across time steps)

#Precision Recall Curve AUC
tergmgof_control$`Tie prediction`$auc.pr
tergmgof_control$`Tie prediction`$auc.roc
#PR AUC is 0.0658 and ROC AUC is 0.497


#Density to compare to AUC
total_edges <- sum(sapply(estnets[2:7], function(net) {
  sum(as.matrix.network(net))
}))

total_dyads <- sum(sapply(estnets[2:7], function(net) {
  length(as.matrix.network(net))
}))

baseline_density <- total_edges / total_dyads
baseline_density
#0.05568599
#In PR analysis, interpret relative to baseline positive rate (i.e. positive edges), random baseline = edge prevalence would be random model. Since 0.066 AUC is above 0.0557 model is better than random.

#Check for TC Model
tergmgof_TC$`Tie prediction`$auc.pr
tergmgof_TC$`Tie prediction`$auc.roc
#now PR AUC is 0.116 and ROC is 0.688 which improves



#Density of the spatial networks to compare to AUC
total_edges <- sum(sapply(estspatialnets[2:7], function(net) {
  sum(as.matrix.network(net))
}))

total_dyads <- sum(sapply(estspatialnets[2:7], function(net) {
  length(as.matrix.network(net))
}))

baseline_density <- total_edges / total_dyads
baseline_density
#Density is higher at 0.075, understandable given smaller network size from excluding projects

#Check for Spatial Benefits Model
tergmgof_spatialben$`Tie prediction`$auc.pr
tergmgof_spatialben$`Tie prediction`$auc.roc
#Now PR AUC is 0.104 and ROC AUC is 0.539 which slightly declines but understandable given insignficant predictors dominating this model
#PR AUC is still higher than baseline density of 0.075

#Check Full Model
tergmgof_spatialfull$`Tie prediction`$auc.pr
tergmgof_spatialfull$`Tie prediction`$auc.roc
#Now PR AUC is 0.2088 which is much improved
#And ROC AUC is 0.725 which is also a big improvement
#Both much higher than baseline density of 0.075

```


```{r Excluded Projects Analysis}
#Create Dataset of Excluded Projects (after habitat filtering, before funding availability filtering)

ExcludedProjects <- subset(RawEcoAtlas4, !(EAid %in% EcoAtlas$EAid))
ExcludedProjects <- ExcludedProjects %>% unite(EAid_noactivity, projectid, siteid, sep = ".", remove = FALSE)
ExcludedProjectsEAID <- ExcludedProjects$EAid_noactivity

projectsshp <- projectsshp %>%  unite(EAid_noactivity, projectid, siteid, sep = ".", remove = FALSE)
ExcludedProjshp <- projectsshp[projectsshp$EAid %in% ExcludedProjectsEAID, ]
ExcludedProjshp <- st_as_sf(ExcludedProjshp)


#Perform Area Interpolation to Get CES and Bio Scores for Excluded Projects

#biodiversity interpolation
#use areal package for this
#validate function to confirm all requirements are there
ar_validate(bio, ExcludedProjshp, varList="bptn_nr", verbose=TRUE)

#need to transform into planar CRS
excludeprojects.utm <-  ExcludedProjshp %>%    st_transform(crs = "+proj=utm +zone=10 +datum=NAD83 +ellps=GRS80")

#complete interpolation on the normalized score of biodiversity for the habitat hexagon relative to the eco region it is in
exclprojbio <- aw_interpolate(excludeprojects.utm, tid = shpid, source = biodiverse.utm, sid = OBJECTID,
                           weight = "sum", output = "sf", 
                          intensive = "SpBioWtEco")
#drop geometry and only save the scores and project id so that we can merge it with existing project attribute data
exclprojbio <- exclprojbio %>% st_drop_geometry() %>% dplyr::select("shpid", "pname", "SpBioWtEco")


#interpolation for the CI score
exclCalEnviro.aw <- aw_interpolate(excludeprojects.utm, tid = shpid, source = ces.utm, sid = Tract,
                           weight = "sum", output = "sf", 
                           intensive = c("CIscore"))
#drop geometry again
exclprojCES <- exclCalEnviro.aw %>% st_drop_geometry() %>% dplyr::select("shpid", "pname", "CIscore")

#convert -999 values to NA
exclprojCES$CalEnvscore <- ifelse(exclprojCES$CIscore<0, NA, exclprojCES$CIscore)

#remove duplicates from bio and CES interpolations 
exclprojbio <- exclprojbio %>% 
  dplyr::filter(!duplicated(cbind(shpid, pname, SpBioWtEco)))

exclprojCES <- exclprojCES %>% dplyr::filter(!duplicated(cbind(shpid, pname, CalEnvscore)))

#merge back into project attributes
ExcludedProjshp <- dplyr::left_join(ExcludedProjshp, exclprojbio, by="shpid")
ExcludedProjshp <- dplyr::left_join(ExcludedProjshp, exclprojCES, by="shpid")

#st_write(ExcludedProjshp, "ExcludedProjs_wAW.shp")

```

```{r Spatial versus Non Spatial Projects Comparision}

attributes2$nospatialdata <- ifelse(attributes2$blankej==1 | attributes2$blankflood==1 | attributes2$blankbio==1, 1, 0)

table(attributes2$nospatialdata) #identified the full 65 projects without sufficient spatial data

NonSpatialProjects <- subset(attributes2, attributes2$nospatialdata==1)
SpatialProjects <- subset(attributes2, attributes2$nospatialdata==0)
SpatialProjects <- subset(SpatialProjects, !is.na(SpatialProjects$EAid))

summary(NonSpatialProjects$startyear)
summary(SpatialProjects$startyear)


table(NonSpatialProjects$deltaregion)
table(SpatialProjects$deltaregion)

table(NonSpatialProjects$habitatname)
table(SpatialProjects$habitatname)


table(NonSpatialProjects$activitytypename)
table(SpatialProjects$activitytypename)


summary(NonSpatialProjects$thousacres)
summary(SpatialProjects$thousacres)
sd(NonSpatialProjects$thousacres, na.rm=TRUE)
sd(SpatialProjects$thousacres, na.rm = TRUE)

summary(NonSpatialProjects$SUM_benefits)
summary(SpatialProjects$SUM_benefits)
sd(NonSpatialProjects$SUM_benefits)
sd(SpatialProjects$SUM_benefits)


summary(NonSpatialProjects$degree)
summary(SpatialProjects$degree)

sd(NonSpatialProjects$degree)
sd(SpatialProjects$degree)

```

```{r degree range for boundary spanners}

boundaryspanners <- subset(attributes2, attributes2$orgboundaryspanner==1)
summary(boundaryspanners$degree)
sd(boundaryspanners$degree)

nonboundaryspanners <- subset(attributes2, attributes2$orgboundaryspanner==0)
summary(nonboundaryspanners$degree)
sd(nonboundaryspanners$degree)


```

```{r explore funding pools over time}
#orgs with most funding pools
table(attributes2$fundpools)
#USFWS (11), CDFW and DWR (6), WCB (5) and rest below 5
#121 with 1 funding pool, 37 funding pools (not funders)

#Degree of high funding pool orgs
highfundpools <- attributes2 %>% filter(fundpools>4) %>% select(nodes, fundpools, degree)

degreebyfundpool <- attributes2 %>% group_by(fundpools) %>% summarise(mean(degree))
#USFWS (75), CDFW (41), DWR (32), WCB (53)
#Average of those with 1 pool- 3.719
#Average of those with 0 pools- 1.838

#ID Degree of high funding pool orgs over time steps (step 0, 1, 2, 3, 4, 5, 6)
View(step0degree) 
View(step1degree) 
View(step2degree) 
View(step3degree) 
...
#degree by org funding pools over time steps

attributes2$nofundpool <- ifelse(attributes2$fundpools==0, 1, ifelse(attributes$fundpools>0, 0, NA))

table(attributes2$nofundpool)

attributes2$onefundpool <- ifelse(attributes$fundpools==1, 1, 0)

attributes2$twoplusfundpool <- ifelse(attributes$fundpools>1, 1, 0)

orgfundingpools <- attributes2 %>% select("nodes", "fundpools", "nofundpool", "onefundpool", "twoplusfundpool")

orgfundingpools$org <- orgfundingpools$nodes

step0degree <- left_join(step0degree, orgfundingpools, by="org")
step1degree <- left_join(step1degree, orgfundingpools, by="org")
step2degree <- left_join(step2degree, orgfundingpools, by="org")
step3degree <- left_join(step3degree, orgfundingpools, by="org")
step4degree <- left_join(step4degree, orgfundingpools, by="org")
step5degree <- left_join(step5degree, orgfundingpools, by="org")
step6degree <- left_join(step6degree, orgfundingpools, by="org")

step0degree %>% group_by(nofundpool) %>% summarise(mean(degree))
step0degree %>% group_by(onefundpool) %>% summarise(mean(degree))
step0degree %>% group_by(twoplusfundpool) %>% summarise(mean(degree))


step1degree %>% group_by(nofundpool) %>% summarise(mean(degree))
step1degree %>% group_by(onefundpool) %>% summarise(mean(degree))
step1degree %>% group_by(twoplusfundpool) %>% summarise(mean(degree))


step2degree %>% group_by(nofundpool) %>% summarise(mean(degree))
step2degree %>% group_by(onefundpool) %>% summarise(mean(degree))
step2degree %>% group_by(twoplusfundpool) %>% summarise(mean(degree))


step3degree %>% group_by(nofundpool) %>% summarise(mean(degree))
step3degree %>% group_by(onefundpool) %>% summarise(mean(degree))
step3degree %>% group_by(twoplusfundpool) %>% summarise(mean(degree))



step4degree %>% group_by(nofundpool) %>% summarise(mean(degree))
step4degree %>% group_by(onefundpool) %>% summarise(mean(degree))
step4degree %>% group_by(twoplusfundpool) %>% summarise(mean(degree))


step5degree %>% group_by(nofundpool) %>% summarise(mean(degree))
step5degree %>% group_by(onefundpool) %>% summarise(mean(degree))
step5degree %>% group_by(twoplusfundpool) %>% summarise(mean(degree))


step6degree %>% group_by(nofundpool) %>% summarise(mean(degree))
step6degree %>% group_by(onefundpool) %>% summarise(mean(degree))
step6degree %>% group_by(twoplusfundpool) %>% summarise(mean(degree))
```

```{r test of funding pools in TERGM}

#create new dummy variables of funding pools

#use two plus fund pools in TERGM
# also create one that is simply 0 fund pools or >1 fund pool

attributes2$binaryfund <- ifelse(attributes2$fundpools==0, 0, ifelse(attributes2$fundpools>0, 1, NA))

network::set.vertex.attribute(net, "multifund", attributes2$twoplusfundpool)
network::set.vertex.attribute(net, "binaryfund", attributes2$binaryfund)


#Now rerun the earlier code to split the network into time steps to now have new attributes from main network

#create new list options of only network steps 3-6 (numbered 2 to 5 when start from 0) to model TERGM for fewer time steps- include prior step 2 as the base which is not estimated

nets3to6 <- list(step2net, step3net, step4net, step5net)

spatialnets3to6 <- list(step2spatial, step3spatial, step4spatial, step5spatial)

set.seed(11334)

#Test 2 TERGMs with binary funding pool attribute (w/ and w/o boundary spanner) for full spatial model and org capacity only

#1- Binary Fund Pool Attribute, Boundary Spanner, Org Capacity Only
TERGM_vNew1 <- btergm(estnets ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1)+ b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("binaryfund") + b2nodematch("gov") + b2nodematch("orgscale"), offset=FALSE)
summary(TERGM_vNew1)


#2- Binary Fund Pool Attribute, No Boundary Spanner, Org Capacity Only
TERGM_vNew2 <- btergm(estnets ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1)+ b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("binaryfund") + b2nodematch("gov") + b2nodematch("orgscale"), offset=FALSE)
summary(TERGM_vNew2)

#3- Binary Fund Pool Attribute, Boundary Spanner, Full Spatial Model
TERGM_vNew3 <- btergm(estspatialnets ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("binaryfund") + b2nodematch("orgscale") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_vNew3)

#4- Binary Fund Pool Attribute, No Boundary Spanner, Full Spatial Model
TERGM_vNew4 <- btergm(estspatialnets ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("binaryfund") + b2nodematch("orgscale") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_vNew4)

#Test 2 TERGMs with 2+ funding pool attributes (w/ and w/o boundary spanner) for full spatial model and org capacity only

#1- 2+ Fund Pool Attribute, Boundary Spanner, Org Capacity Only
TERGM_vNew5 <- btergm(estnets ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1)+ b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("multifund") + b2nodematch("gov") + b2nodematch("orgscale"), offset=FALSE)
summary(TERGM_vNew5)

#2- 2+ Fund Pool Attribute, No Boundary Spanner, Org Capacity Only
TERGM_vNew6 <- btergm(estnets ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1)+ b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("multifund") + b2nodematch("gov") + b2nodematch("orgscale"), offset=FALSE)
summary(TERGM_vNew6)

#3- 2+ Fund Pool Attribute, Boundary Spanner, Full Spatial Model
TERGM_vNew7 <- btergm(estspatialnets ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("multifund") + b2nodematch("orgscale") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_vNew7)

#4- 2+ Fund Pool Attribute, No Boundary Spanner, Full Spatial Model
TERGM_vNew8 <- btergm(estspatialnets ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("multifund") + b2nodematch("orgscale") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_vNew8)

#Test 1 TERGM dropping boundary spanner (original fund pools variable) for full spatial model and org capacity only

#1- No boundary spanner in original org capacity only TERGM
TERGM_vNew9 <- btergm(estnets ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1)+ b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("fundingpools") + b2nodematch("gov") + b2nodematch("orgscale"), offset=FALSE)
summary(TERGM_vNew9)

#2- No boundary spanner in original full spatial TERGM
TERGM_vNew10 <- btergm(estspatialnets ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("fundingpools") + b2nodematch("orgscale") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_vNew10)

#Test 6 variations of the TERGMs for only time steps 3-5 (w/ and w/o boundary spanner and for all 3 potential funding pool variables) for full spatial model and org capacity only

#1- Limited time step, binary fund pool, with boundary spanner, org capacity only
TERGM_vN11 <- btergm(nets3to6 ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1)+ b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("binaryfund") + b2nodematch("gov") + b2nodematch("orgscale"), offset=FALSE)
summary(TERGM_vN11)

#2- Limited time step, binary fund pool, no boundary spanner, org capacity only
TERGM_vN12 <- btergm(nets3to6 ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1)+ b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("binaryfund") + b2nodematch("gov") + b2nodematch("orgscale"), offset=FALSE)
summary(TERGM_vN12)

#3- Limited time step, binary fund pool, with boundary spanner, full spatial
TERGM_vNew13 <- btergm(spatialnets3to6 ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("binaryfund") + b2nodematch("orgscale") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_vNew13)

#4- Limited time step, binary fund pool, no boundary spanner, full spatial
TERGM_vNew14 <- btergm(spatialnets3to6 ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("binaryfund") + b2nodematch("orgscale") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_vNew14)

#5- Limited time step, 2+ fund pool, with boundary spanner, org capacity only
TERGM_vN15 <- btergm(nets3to6 ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1)+ b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("multifund") + b2nodematch("gov") + b2nodematch("orgscale"), offset=FALSE)
summary(TERGM_vN15)

#6- Limited time step, 2+ fund pool, no boundary spanner, org capacity only
TERGM_vN16 <- btergm(nets3to6 ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1)+ b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("multifund") + b2nodematch("gov") + b2nodematch("orgscale"), offset=FALSE)
summary(TERGM_vN16)

#7- Limited time step, 2+ fund pool, with boundary spanner, full spatial
TERGM_vNew17 <- btergm(spatialnets3to6 ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("multifund") + b2nodematch("orgscale") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_vNew17)

#8- Limited time step, 2+ fund pool, no boundary spanner, full spatial
TERGM_vNew18 <- btergm(spatialnets3to6 ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2factor("orgscale") + b2cov("implementer") +  b2cov("multifund") + b2nodematch("orgscale") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_vNew18)

#9- Limited time step, og. fund pool, with boundary spanner, org capacity only
TERGM_vN19 <- btergm(nets3to6 ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1)+ b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("fundingpools") + b2nodematch("gov") + b2nodematch("orgscale"), offset=FALSE)
summary(TERGM_vN19)

#10- Limited time step, og. fund pool, no boundary spanner, org capacity only
TERGM_vN20 <- btergm(nets3to6 ~ edges + gwb1degree(0.5, fixed=TRUE)+gwb2degree(0.5, fixed=TRUE) + b2degree(1)+ b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("fundingpools") + b2nodematch("gov") + b2nodematch("orgscale"), offset=FALSE)
summary(TERGM_vN20)

#11- Limited time step, og. fund pool, with boundary spanner, full spatial
TERGM_vNew21 <- btergm(spatialnets3to6 ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("boundaryspanner") + b2cov("fundingpools") + b2nodematch("orgscale") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_vNew21)

#12- Limited time step, og. fund pool, no boundary spanner, full spatial
TERGM_vNew22 <- btergm(spatialnets3to6 ~ edges+ gwb1degree(0.5, fixed=TRUE) + gwb2degree(0.5,fixed=TRUE) + b2degree(1) + b2cov("gov") + b2factor("orgscale") + b2cov("implementer") + b2cov("fundingpools") + b2nodematch("orgscale") + b2nodematch("gov") + b1cov("totalES") + b1cov("tidal") + b1cov("delta") + b1cov("floodrisk") + b1cov("CES") + b1cov("biodiversity") + b1cov("thousacres"), offset=FALSE)
summary(TERGM_vNew22)

```


```{r extra correlations with funding pool}
#Correlation funding dummies and boundary spanner
cor.test(attributes2$orgboundaryspanner, attributes2$binaryfund, method="spearman")
cor.test(attributes2$orgboundaryspanner, attributes2$twoplusfundpool, method= "spearman")
cor.test(attributes2$orgboundaryspanner, attributes2$fundpools, method= "spearman")

#correlations for only spatial model orgs
spatialorgs <- spatialnet %v% 'vertex.names'
spatialorgs <- spatialorgs[161:298] #138 orgs compared to 169 in overall network

spatialorgatts <- subset(attributes2, nodes %in% spatialorgs)

cor.test(spatialorgatts$orgboundaryspanner, spatialorgatts$binaryfund, method="spearman")
cor.test(spatialorgatts$orgboundaryspanner, spatialorgatts$twoplusfundpool, method= "spearman")
cor.test(spatialorgatts$orgboundaryspanner, spatialorgatts$fundpools, method= "spearman")
```

```{r average degree by time step by funding pools cont.}
#average degree of 0 FP, >1 FP for spatial model orgs by time step

#first get degrees for only spatial network in each time step
spatialdegstep0 <- sna::degree(step0spatial)
spatialnamesstep0 <- step0spatial %v% 'vertex.names'
spatialdegstep0 <- cbind(spatialnamesstep0, spatialdegstep0)
spatialdegstep0 <- as.data.frame(spatialdegstep0)
spatialdegstep0$degree <- as.numeric(spatialdegstep0$spatialdegstep0)
spatialdegstep0$degree <- spatialdegstep0$degree/2  #must divide by 2 for bipartite


spatialdegstep1 <- sna::degree(step1spatial)
spatialnamesstep1 <- step1spatial %v% 'vertex.names'
spatialdegstep1 <- cbind(spatialnamesstep1, spatialdegstep1)
spatialdegstep1 <- as.data.frame(spatialdegstep1)
spatialdegstep1$degree <- as.numeric(spatialdegstep1$spatialdegstep1)
spatialdegstep1$degree <- spatialdegstep1$degree/2  #must divide by 2 for bipartite

spatialdegstep2 <- sna::degree(step2spatial)
spatialnamesstep2 <- step2spatial %v% 'vertex.names'
spatialdegstep2 <- cbind(spatialnamesstep2, spatialdegstep2)
spatialdegstep2 <- as.data.frame(spatialdegstep2)
spatialdegstep2$degree <- as.numeric(spatialdegstep2$spatialdegstep2)
spatialdegstep2$degree <- spatialdegstep2$degree/2  #must divide by 2 for bipartite


spatialdegstep3 <- sna::degree(step3spatial)
spatialnamesstep3 <- step3spatial %v% 'vertex.names'
spatialdegstep3 <- cbind(spatialnamesstep3, spatialdegstep3)
spatialdegstep3 <- as.data.frame(spatialdegstep3)
spatialdegstep3$degree <- as.numeric(spatialdegstep3$spatialdegstep3)
spatialdegstep3$degree <- spatialdegstep3$degree/2  #must divide by 2 for bipartite



spatialdegstep4 <- sna::degree(step4spatial)
spatialnamesstep4 <- step4spatial %v% 'vertex.names'
spatialdegstep4 <- cbind(spatialnamesstep4, spatialdegstep4)
spatialdegstep4 <- as.data.frame(spatialdegstep4)
spatialdegstep4$degree <- as.numeric(spatialdegstep4$spatialdegstep4)
spatialdegstep4$degree <- spatialdegstep4$degree/2  #must divide by 2 for bipartite



spatialdegstep5 <- sna::degree(step5spatial)
spatialnamesstep5 <- step5spatial %v% 'vertex.names'
spatialdegstep5 <- cbind(spatialnamesstep5, spatialdegstep5)
spatialdegstep5 <- as.data.frame(spatialdegstep5)
spatialdegstep5$degree <- as.numeric(spatialdegstep5$spatialdegstep5)
spatialdegstep5$degree <- spatialdegstep5$degree/2  #must divide by 2 for bipartite



spatialdegstep6 <- sna::degree(step6spatial)
spatialnamesstep6 <- step6spatial %v% 'vertex.names'
spatialdegstep6 <- cbind(spatialnamesstep6, spatialdegstep6)
spatialdegstep6 <- as.data.frame(spatialdegstep6)
spatialdegstep6$degree <- as.numeric(spatialdegstep6$spatialdegstep6)
spatialdegstep6$degree <- spatialdegstep6$degree/2  #must divide by 2 for bipartite

#now merge funding pools with orgs in each time step

#first rename nodes column to match
spatialdegstep0 <- spatialdegstep0 %>% rename(org = spatialnamesstep0)
spatialdegstep1 <- spatialdegstep1 %>% rename(org = spatialnamesstep1)
spatialdegstep2 <- spatialdegstep2 %>% rename(org = spatialnamesstep2)
spatialdegstep3 <- spatialdegstep3 %>% rename(org = spatialnamesstep3)
spatialdegstep4 <- spatialdegstep4 %>% rename(org = spatialnamesstep4)
spatialdegstep5 <- spatialdegstep5 %>% rename(org = spatialnamesstep5)
spatialdegstep6 <- spatialdegstep6 %>% rename(org = spatialnamesstep6)


#then combine so each org has degree and funding pools per time step
spatialdegstep0 <- left_join(spatialdegstep0, orgfundingpools, by="org")
spatialdegstep1 <- left_join(spatialdegstep1, orgfundingpools, by="org")
spatialdegstep2 <- left_join(spatialdegstep2, orgfundingpools, by="org")
spatialdegstep3 <- left_join(spatialdegstep3, orgfundingpools, by="org")
spatialdegstep4 <- left_join(spatialdegstep4, orgfundingpools, by="org")
spatialdegstep5 <- left_join(spatialdegstep5, orgfundingpools, by="org")
spatialdegstep6 <- left_join(spatialdegstep6, orgfundingpools, by="org")

#use group_by to calculate average degree by time step
spatialstep0degbyfp <- spatialdegstep0 %>% group_by(nofundpool) %>% summarise(mean(degree))
spatialstep0degby2fp <- spatialdegstep0 %>% group_by(twoplusfundpool) %>% summarise(mean(degree))

spatialstep1degbyfp <- spatialdegstep1 %>% group_by(nofundpool) %>% summarise(mean(degree))
spatialstep1degby2fp <- spatialdegstep1 %>% group_by(twoplusfundpool) %>% summarise(mean(degree))

spatialstep2degbyfp <- spatialdegstep2 %>% group_by(nofundpool) %>% summarise(mean(degree))
spatialstep2degby2fp <- spatialdegstep2 %>% group_by(twoplusfundpool) %>% summarise(mean(degree))

spatialstep3degbyfp <- spatialdegstep3 %>% group_by(nofundpool) %>% summarise(mean(degree))
spatialstep3degby2fp <- spatialdegstep3 %>% group_by(twoplusfundpool) %>% summarise(mean(degree))

spatialstep4degbyfp <- spatialdegstep4 %>% group_by(nofundpool) %>% summarise(mean(degree))
spatialstep4degby2fp <- spatialdegstep4 %>% group_by(twoplusfundpool) %>% summarise(mean(degree))

spatialstep5degbyfp <- spatialdegstep5 %>% group_by(nofundpool) %>% summarise(mean(degree))
spatialstep5degby2fp <- spatialdegstep5 %>% group_by(twoplusfundpool) %>% summarise(mean(degree))

spatialstep6degbyfp <- spatialdegstep6 %>% group_by(nofundpool) %>% summarise(mean(degree))
spatialstep6degby2fp <- spatialdegstep6 %>% group_by(twoplusfundpool) %>% summarise(mean(degree))


#Average Degree for all orgs in spatial network by time step
meandegspatialstep0 <- spatialdegstep0 %>% filter(!is.na(fundpools)) %>% summarise(mean(degree))

meandegspatialstep1 <- spatialdegstep1 %>% filter(!is.na(fundpools)) %>% summarise(mean(degree))


meandegspatialstep2 <- spatialdegstep2 %>% filter(!is.na(fundpools)) %>% summarise(mean(degree))

meandegspatialstep3 <- spatialdegstep3 %>% filter(!is.na(fundpools)) %>% summarise(mean(degree))

meandegspatialstep4 <- spatialdegstep4 %>% filter(!is.na(fundpools)) %>% summarise(mean(degree))

meandegspatialstep5 <- spatialdegstep5 %>% filter(!is.na(fundpools)) %>% summarise(mean(degree))

meandegspatialstep6 <- spatialdegstep6 %>% filter(!is.na(fundpools)) %>% summarise(mean(degree))
```