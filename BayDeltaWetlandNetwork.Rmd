...
```{r additional GOF}
#for each of the 3 spatial and 3 nonspatial TERGMs include PR-AUC

#Precision Recall Curve

library(PRROC)

compute_pr_curve <- function(model) {
  # 1. Extract design matrix and response
  X <- as.matrix(model@effects)
  y <- as.numeric(model@response)
  ...
  # PRROC expects "scores for positive class" and labels
  pr <- pr.curve(scores.class0 = p[y == 1],
                 scores.class1 = p[y == 0],
                 curve = TRUE)
  
  # Plot the PR curve
  plot(pr, main = paste("PR Curve for", deparse(model@formula)))
  
  # Return the PR object for area under the curve etc.
  return(pr)
}


#For control model
pr_TERGM_c <- compute_pr_curve(TERGM_control)
pr_TERGM_c$auc.integral  # area under the PR curve

# For TC model
pr_TERGM_TC <- compute_pr_curve(TERGM_TC)
pr_TERGM_TC$auc.integral  # area under the PR curve

#For spatial benefits model
pr_TERGM_b <- compute_pr_curve(TERGM_spatialbens)
pr_TERGM_b$auc.integral  # area under the PR curve

#for full model
pr_TERGM_f <- compute_pr_curve(TERGM_spatialfull)
pr_TERGM_f$auc.integral  # area under the PR curve
